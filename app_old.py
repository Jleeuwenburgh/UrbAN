...
# Choropleth of all municipalities with their relative entropy values
GEMEENTEN = gpd.read_parquet("data/gemeenten_RE.parquet")

# gemeenten['geometry'] = gemeenten['geometry'].simplify(0.0005)
GEMEENTEN["geometry"] = (
    ...
    [
        ...
        dbc.Row(
            children=[
                ...
                dbc.Container(
                    id="RE_graphcontainer",
                ),
            ],
            id="relative_entropy_row",
        ),
...
@app.callback(
    Output("RE_graphcontainer", "children"),
    [
        Input("relative_entropy_filters", "value"),
        Input("relative_entropy_levels", "value"),
    ],
)
def generate_RE_graph(filter_value, level_value):
    fig = px.choropleth(
        GEMEENTEN,
        geojson=GEMEENTEN.geometry,
        locations=GEMEENTEN.index,
        color=f"RE_L{level_value}_{filter_value}",
        hover_name="gemeentenaam",
        color_continuous_scale="OrYel",
        hover_data={"gemeentenaam": True, "total_amenities_0": True},
    )
    fig.update_geos(fitbounds="locations", visible=False)
